...
for (b in bands) {
  ...
  # Build full model
  valence.model = lmer(parietal_mean ~ valence_rating + arousal_rating +
  ...
  valence.null = lmer(parietal_mean ~ arousal_rating +
                        (1+valence_rating|participant) +
                        (1+valence_rating|video_id) +
                        (1+valence_rating|second),
                      data=band_data,
...
for (b in bands) {
  ...
  # Build full model
  arousal.model = lmer(parietal_mean ~ arousal_rating + valence_rating +
  ...
  # Build reduced model without valence ratings
  arousal.null = lmer(parietal_mean ~ valence_rating +
                        (1+arousal_rating|participant) +
                        (1+arousal_rating|video_id) +
                        (1+arousal_rating|second),
                      data=band_data,
