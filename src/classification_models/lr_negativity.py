...
def lr_negativity():
    ...
    for p in exp_participant_codes:
        ...
        data = negativity.iloc[:, 1:3].values
        labels = negativity.iloc[:, 0].values
    ...
    all_participants_df.to_csv(d + '/deap/results/metrics/lr_negativity.csv', index=False)
    ...
    means_std.to_csv(d + '/reports/metrics/lr_negativity_means.csv', index=True)

    print('Metrics for the negativity LR model were exported to /reports/metrics/')
