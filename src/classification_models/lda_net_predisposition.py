...
def lda_net_predisposition():
    # Run once per participant
    all_participants = []
    for p in exp_participant_codes:
        print('Analysing participant %s' % (p))
        participant_features = pd.read_csv(d + '/features/%s_features.csv' % (p))
    ...
    means_std.to_csv(d + '/reports/metrics/lda_net_predisposition_means.csv', index=True)

    print('Metrics for the net predisposition LDA model were exported to /reports/metrics/')
