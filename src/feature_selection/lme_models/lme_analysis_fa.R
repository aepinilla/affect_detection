...
working_directory <- "~/Documents/MATLAB/affect_detection/reports/feature_selection/lme/fa/"
#########################################
...
features <- list('frontal_asymmetry')
...
select_data <- function(data, iteration_trials, dimension, power_band) {
  if (dimension == 'negativity_rating') {
    negativity_trials <- iteration_trials$negativity
    subset <- data %>%
      filter(band == power_band,
...
for (p in len_participants_list) {
  ...
  for (i in iterations) {
    ...
    for (d in dimensions) {
      ...
      for (b in bands) {
        subset_df <- select_data(participant_data, iteration_trials, d, b)
        for (f in features) {
          ...
          if ((class(null_model) == 'lmerMod')  & (class(full_model) == 'lmerMod')) {
            ...
            participant_results[[sprintf('%s_%s_%s_%s', i, d, b, f)]] <- result
    ...
    sapply(seq_along(1:length(participant_results)), 
           function(n) write_feather(participant_results[[n]], 
                                     paste(export_path,"_fa_",i,'_',n,".feather", sep = "")))
  }
}
